...
#include <string.h>
  ...
  */
  ...
  const int ADDR_SIZE_DB =
      END_DB;  // Store address of numbers of data in database

  const int ADDR_SIZE_METADATA =
      ADDR_SIZE_DB - 2;  // Store address of where meta-data end in database

  const int ADDR_NB_TABLES =
      ADDR_SIZE_METADATA - 2;  // Store address of the numbers of tables

  const int ADDR_TABLES_METADATA =
      ADDR_NB_TABLES -
      1;  // Address where we start the storage of tables metadata

  const int BYTES_TABLES_GEN_METADATA =
      8;  // AMount of bytes used to store table data
  ...
  */
  ...
  private:
    ...
    int addr =
        STATUS_TABLE_NOT_FOUND;  // Address of the table in meta data section
  ...
  public:
    ...
    * @param capacity The maximum data that can be hold by this table. If this value is attended, we override old value from the beginning
    * @param col The number of columns of this table
    * @param tableCol The informations about columns: name and size
    * @return uint8_t We return a status: if table is created, or already existed, if capacity is exceeded or something else... - see status code
    */
    int CREATE_TABLE(char* tableName, uint8_t capacity, uint8_t col,
                    Column tableCol[]);

    /********************************************************************************************************************************/
    /**
     * @brief Allow us to check the existence of a table by retrieving its addr. If negative address is returned,
     * that mean, not exist.
     * 
     * @param tableName The name of table (max 2 char)
     * @return int Return its address. Positif if it existed and so where the table meta data started and negative if 
     * it is not existed
     */
    ...
    /********************************************************************************************************************************/
    /**
     * @brief This function is used if we wan to set/alter data on the table, insert or update or delete
     * 
     * @param tableName The table name
     * @return TableData& A reference to a member data of MemoryAManager to allow chaining when working on the table
     */
     ...
     * @return TableData& TableData& A reference to a member data of MemoryAManager to allow chaining when working on the table
     */
  ...
  private:
    ...
    /**
     * @brief Set the meta data end emplacement after adding to the read value the new amount of meta data
     * used to store new table infos
     * 
     * @param nSizeMeta 
     */
    ...
    /********************************************************************************************************************************/
    /**
     * @brief It is a TableData used by MemoryManager that user can access to chain functions when working on a table
     * 
     */
    TableData T_DATA;
  };
  ...
  void MemoryManager::setSizeMeta(int nSizeMeta) {
    EEPROM.put(ADDR_SIZE_METADATA, nSizeMeta);
  }
  ...
  void MemoryManager::setNbTables(uint8_t nbTab) {
    EEPROM.write(ADDR_NB_TABLES, nbTab);
  }
  ...
  int MemoryManager::CREATE_TABLE(char* tableName, uint8_t capacity, uint8_t col,
                                  Column tableCol[]) {
    
    if (capacity > MAX_CAP) return STATUS_MAX_CAP_EXCEEDED;

    int dataMemoryNeed =
        0;  // to calculate new space needs to stores this table data
